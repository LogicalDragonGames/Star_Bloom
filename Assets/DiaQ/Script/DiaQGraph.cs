...
namespace DiaQ
{
	...
	public class DiaQGraph
	{
		...
		[SerializeField] public readonly float zoomMin = 0.2f;
		[SerializeField] public readonly float zoomMax = 1.0f;
		[SerializeField] public Rect viewport = new Rect( 0f, 0f, 0f, 0f );
		[SerializeField] public float zoom = 1.0f;
		[SerializeField] public Vector2 zoomCoordsOrigin = Vector2.zero;
		...
		{	// recursively walks the nodes of the graph up to where player interaction is 
			...
			switch (currNode.type)
			{
				...
				case DiaQNode.Type.DecisionTree:
				{
					currNode = currNode.ExecuteDecisionTree(this);
				} break;
		...
		private void HandleSetVar()
		{
			...
			else if (currNode.i_data[0] == 3)	// offset
			{
				string curValStr = DiaQEngine.Instance.GetDiaQVarValue( currNode.data[0] );
				float curVal = float.Parse( curValStr );
				float offsetVal = float.Parse( currNode.data[1] );

				DiaQEngine.Instance.SetDiaQVarValue( currNode.data[0], (curVal+offsetVal).ToString() );
			}
		...
		public DiaQNode CreateNode(DiaQNode.Type type, Vector2 pos)
		{
		...
		public Vector2 DrawOffset { get { return new Vector2( viewport.x, viewport.y ); } set { viewport.x = value.x; viewport.y = value.y; } }
