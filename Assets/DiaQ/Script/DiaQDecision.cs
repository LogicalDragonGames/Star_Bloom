		...
		private bool DiaQVariableTest()
		{
			...
			if (valType == ValType.Empty)
			{
				if (testType == TestType.Equal) 
					return string.IsNullOrEmpty(v) == b_val;
				else if (testType == TestType.NotEqual) 
					return string.IsNullOrEmpty(v) != b_val;
			}
			...
			{
				if (testType == TestType.Equal) 
					return string.IsNullOrEmpty(v2) == b_val;
				else if (testType == TestType.NotEqual) 
					return string.IsNullOrEmpty(v2) != b_val;
			}
			else
			{
				int comparison = v.CompareTo( v2 );
				switch( testType )
				{
				case TestType.Equal:
					return (0 == comparison) == b_val;
					break;
				...
				case TestType.Bigger:
					return (comparison > 0) == b_val;
					break;
				...
				case TestType.BiggerEqual:
					return (comparison >= 0) == b_val;
					break;

				case TestType.SmallerEqual:
					return (comparison <= 0) == b_val;
					break;
				}
			}
