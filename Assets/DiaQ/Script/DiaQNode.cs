...
namespace DiaQ
{
	...
	public class DiaQNode
	{
		...
		public DiaQDecisionTree decisionTree = null;
		...
		/// <summary>
		/// Called on a DecisionTree type node. It will execute tests in order of listed priority,
		/// returning the first successful test node OR the else condition if none succeeded
		/// </summary>
		public DiaQNode ExecuteDecisionTree(DiaQGraph graph)
		{
			int slot = decisionTree.Evaluate(graph);
		...
		public void Init(int id, Type type)
		{
			...
			switch (type)
			{
				...
				{
				decisionTree = new DiaQDecisionTree();	
				} break;
		...
		public static Rect GetNodeDefaultRect(DiaQNode node, Vector2 pos)
		{
			...
			switch (node.type)
			{
				...
				case Type.DecisionTree: r.width = 180f; r.height = 60f; break;
		...
		{
			Rect p = position;
			p.x += graph.viewport.x;
			p.y += graph.viewport.y;
			return p;
		}
		...
		{
			position = r;
			position.x -= graph.viewport.x;
			position.y -= graph.viewport.y;
		}
