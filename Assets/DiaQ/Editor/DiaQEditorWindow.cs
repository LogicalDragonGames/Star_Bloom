...
namespace DiaQEditor
{
	public class DiaQEditorWindow : EditorWindow
	{
		...
		public void OnGUI()
		{
			...
			Rect fixedMainRect = new Rect( mainRect );

			if( null != currGraph )
			{
				mainRect = ZoomRegion.Begin( currGraph.zoom, mainRect );
			}
			...
			if (currGraph!=null) 
			{
				currGraph.viewport.width = splitterPos[0];
				currGraph.viewport.height = position.height;
				DrawGraphArea();
			}
			...
			if (Event.current != null)
			{
				switch (Event.current.type)
				{
					...
					case EventType.MouseDrag:
					{
						...
						{							
							currGraph.viewport.x += Event.current.delta.x * (1f/currGraph.zoom);
							currGraph.viewport.y += Event.current.delta.y * (1f/currGraph.zoom);
							Event.current.Use();
						}
				...
				if (Event.current.type == EventType.ScrollWheel && null != currGraph)
				{
					Vector2 screenCoordsMousePos = Event.current.mousePosition;
					Vector2 delta = Event.current.delta;
					//Vector2 zoomCoordsMousePos = ConvertScreenCoordsToZoomCoords(screenCoordsMousePos);
					float zoomDelta = -delta.y / 150.0f;
					float oldZoom = currGraph.zoom;
					currGraph.zoom += zoomDelta;
					currGraph.zoom = Mathf.Clamp( currGraph.zoom, currGraph.zoomMin, currGraph.zoomMax );
					zoomDelta = currGraph.zoom - oldZoom;
					Vector2 mouseCenterOffset = screenCoordsMousePos - mainRect.center;
					currGraph.zoomCoordsOrigin += (-mouseCenterOffset)*zoomDelta - (mainRect.center*zoomDelta);
					//currGraph.zoomCoordsOrigin += mouseCenterOffset * zoomDelta; 
					Event.current.Use();
				}
		...
		{
			if( currGraph != null )
				return (screenCoords - currGraph.viewport.TopLeft()) / currGraph.zoom + currGraph.zoomCoordsOrigin;
			else
				return Vector2.zero;
		}
		...
		private void DrawGraphToolbar()
		{
			...
			{
				GUILayout.Space(15);
				if (GUILayout.Button(new GUIContent(DiaQEdGUI.Icon_ResetView, "Reset view"), DiaQEdGUI.ButtonLeftStyle, GUILayout.Width(30)) && currGraph != null)
				{
					currGraph.zoom = 1f; 
					currGraph.viewport.x = 0f;
					currGraph.viewport.y = 0f;
					currGraph.zoomCoordsOrigin = Vector2.zero;
					linkNode = null; 
				}
				...
				if (GUILayout.Button(new GUIContent(DiaQEdGUI.Icon_DecisionTree, "Add DecisionTree Node"), DiaQEdGUI.ButtonMidStyle, GUILayout.Width(30))) { currGraph.CreateNode(DiaQNode.Type.DecisionTree, new Vector2(splitterPos[0] * 0.5f, this.position.height * 0.5f)); linkNode = null; }
		...
		private void ShowGraphAreaPopMenu()
		{
			...
			menu.AddItem(new GUIContent("Add DecisionTree Node"), false, OnGraphPopMenu, new object[] { 13, Event.current.mousePosition });
		...
		private void OnGraphPopMenu(object arg)
		{
			...
			switch (opt)
			{
				...
				case 2: currGraph.CreateNode(DiaQNode.Type.DecisionTree, (Vector2)args[1]); break;
				...
				case 13: currGraph.CreateNode(DiaQNode.Type.DecisionTree, (Vector2)args[1]); break;
		...
		private void NodeWindow(int id)
		{
			...
			switch (node.type)
			{
				...
				case DiaQNode.Type.DecisionTree: DrawNode_DecisionTree(node); break;
		...
		private void DrawNode_DecisionTree(DiaQNode node)
		{
			GUI.DrawTexture(new Rect(node.position.width - 14, 2, 12, 12), DiaQEdGUI.Icon_DecisionTree);

			int outputPosItr = 0;
			Vector2 v = Vector2.zero;
			int maxWidthOffset = 0;
			Rect defaultSize = DiaQNode.GetNodeDefaultRect(node, Vector2.zero); // TODO: Faster impl

			for( ; outputPosItr < node.decisionTree.tests.Count; ++outputPosItr )
			{
				DiaQDecisionTest test = node.decisionTree.tests[outputPosItr];
				string label = "";

				if( test.varType == DiaQDecisionTest.VarType.DiaQVariable )
					label += test.s_opt1[0] + " ";
				else
					label += test.varType.ToString() + " ";

				label += DiaQDecisionTreeEd.TestTypeNames[(int)test.testType] + " ";

				if( test.valType == DiaQDecisionTest.ValType.String ||
				   test.valType == DiaQDecisionTest.ValType.DiaQVar )
					label += test.s_opt2[0];
				else
					label += test.valType.ToString();

				Vector2 labelSize = GUI.skin.label.CalcSize(new GUIContent(label));

				// Adding 30 for the icon and spacing to the right
				maxWidthOffset = Mathf.Max((int)labelSize.x+30, maxWidthOffset);

				v = node.OutputPosition( outputPosItr );
				GUI.Label(new Rect(0, v.y - 9, node.position.width - 20, 16), label, DiaQEdGUI.RightAlignTextStyle);
				if (GUI.Button(new Rect(v.x - 16, v.y - 8, 16, 16), new GUIContent(DiaQEdGUI.Icon_ArrowGreen), DiaQEdGUI.IconButtonStyle))
				{
					linkNode = node;
					linkOutSlot = outputPosItr;
					linkOutPos1 = node.GetPositionInGraph(currGraph) + v;
				}
			}
	
			// Move the output iterator past the evaluation links
			outputPosItr = node.decisionTree.tests.Count;
			v = node.OutputPosition( outputPosItr );
			GUI.Label(new Rect(0, v.y - 9, node.position.width - 20, 16), "NONE", DiaQEdGUI.RightAlignTextStyle);
			if (GUI.Button(new Rect(v.x - 16, v.y - 8, 16, 16), new GUIContent(DiaQEdGUI.Icon_ArrowRed), DiaQEdGUI.IconButtonStyle))
			{
				linkNode = node;
				linkOutSlot = outputPosItr;
			...
			node.position.width = Mathf.Max( defaultSize.width, maxWidthOffset );
			node.position.height = defaultSize.height + node.decisionTree.tests.Count*20;
		}
		...
		private void DrawGraphProperties()
		{
			GUILayout.BeginHorizontal();
			{
				...
				if (GUILayout.Button("Duplicate", EditorStyles.miniButton, GUILayout.Width(65)))
				{
					DuplicateCurrentGraph();
				}
				
				else if (GUILayout.Button("Delete", EditorStyles.miniButton, GUILayout.Width(65)))
				{
			...
			{
				GUILayout.FlexibleSpace();
				if (GUILayout.Button("Import", EditorStyles.miniButton, GUILayout.Width(65)))
				{
					ImportGraph( currGraph );
					EditorUtility.SetDirty( asset );
				}
				
				else if (GUILayout.Button("Export", EditorStyles.miniButton, GUILayout.Width(65)))
				{
					ExportGraph( currGraph );
				}
			}
			GUILayout.EndHorizontal();
		...
		private void DrawNodeProperties()
		{
			...
			switch (currNode.type)
			{
				...
				case DiaQNode.Type.DecisionTree: DiaQDecisionTreeEd.OnGUI(this, currNode.decisionTree, asset, inspectorRect.width); break;
		...
		private void DrawSetVariableProps()
		{
			...
			GUILayout.BeginHorizontal();
			{
				if( DiaQEdGUI.ToggleButton(currNode.i_data[0] == 3, " offset", EditorStyles.toggle, GUILayout.Width(100)) )
				{
					currNode.data[1] = "";
					currNode.i_data[0] = 3;
				}
		...
		// ============================================================================================================
		#region misc
	public static void ImportGraph( DiaQGraph graph, string path = "" )
	{
		if( null == currGraph )
			return;
		
		if( !EditorUtility.DisplayDialog("Import the Graph?", "This action can't be undone.", "Yes", "Cancel"))
			return;

		// If there is no default path, open a save dialog
		if( path.Length == 0 )
			path = EditorUtility.OpenFilePanel( "Import XML", "", "xml" );

		// If the user didn't specify a proper path, cancel
		if( path.Length == 0 )
			return;

		graph.nodes.Clear();

		Zephyr.XML parser = new Zephyr.XML( path, Zephyr.XML.ParseType.Read );

		// Start the graph input
		parser.Reader.ReadToFollowing( "graph" );
		graph.name = parser.Reader.GetAttribute( "name" );
		graph.customIdent = parser.Reader.GetAttribute( "customIdent" );
		graph.HasSeenPlayer = bool.Parse( parser.Reader.GetAttribute( "playerSeen" ) );
		
		// Start the node list input
		parser.Reader.ReadToFollowing( "node_list" );
		int nodeListCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
		for( int i = 0; i < nodeListCount; ++i )
		{
			parser.Reader.ReadToFollowing( "node" );

			int nodeID = int.Parse( parser.Reader.GetAttribute( "id" ) );
			float positionX = float.Parse( parser.Reader.GetAttribute( "x" ) );
			float positionY = float.Parse( parser.Reader.GetAttribute( "y" ) );
			float positionWidth = float.Parse( parser.Reader.GetAttribute( "w" ) );
			float positionHeight = float.Parse( parser.Reader.GetAttribute( "h" ) );
			DiaQNode.Type nodeType = (DiaQNode.Type) System.Enum.Parse( typeof(DiaQNode.Type), parser.Reader.GetAttribute( "type" ) );
			string nodeComment = parser.Reader.GetAttribute( "comment" );

			DiaQNode node = graph.CreateNode( nodeType, Vector2.zero );
			node.id = nodeID;
			node.position.x = positionX;
			node.position.y = positionY;
			node.position.width = positionWidth;
			node.position.height = positionHeight;
			node.type = nodeType;
			node.comment = nodeComment;
			
			parser.Reader.ReadToFollowing( "data" );
			int nodeDataCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
			node.data = new string[nodeDataCount];
			for( int strDat = 0; strDat < node.data.Length; ++strDat )
			{
				parser.Reader.ReadToFollowing( "value" );
				node.data[strDat] = parser.Reader.GetText();
			}
			
			parser.Reader.ReadToFollowing( "toggle" );
			int nodeToggleCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
			node.toggle = new bool[nodeToggleCount];
			for( int iToggle = 0; iToggle < node.toggle.Length; ++iToggle )
			{
				parser.Reader.ReadToFollowing( "value" );
				node.toggle[iToggle] = parser.Reader.GetBool();
			}
			
			parser.Reader.ReadToFollowing( "i_data" );
			int nodeIDatCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
			node.i_data = new int[nodeIDatCount];
			for( int iDat = 0; iDat < node.i_data.Length; ++iDat )
			{
				parser.Reader.ReadToFollowing( "value" );
				node.i_data[iDat] = parser.Reader.GetInt();
			}
			
			parser.Reader.ReadToFollowing( "f_data" );
			int nodeFDatCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
			node.f_data = new float[nodeFDatCount];
			for( int fDat = 0; fDat < node.f_data.Length; ++fDat )
			{
				parser.Reader.ReadToFollowing( "value" );
				node.f_data[fDat] = parser.Reader.GetInt();
			}
			
			
			// TODO: Serialize prefab reference
			
			
			parser.Reader.ReadToFollowing( "choices" );
			int nodeChoicesCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
			for( int iChoice = 0; iChoice < nodeChoicesCount; ++iChoice )
			{
				parser.Reader.ReadToFollowing( "value" );
				node.choices.Add( parser.Reader.GetText() );
			}

			List<DiaQDecisionTest> tests = null;
			if( node.type == DiaQNode.Type.Decision )
				tests = node.decision.tests;
			else if( node.type == DiaQNode.Type.DecisionTree )
				tests = node.decisionTree.tests;
			
			if( null != tests )
			{
				parser.Reader.ReadToFollowing( "tests" );
				int testCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
				tests.Clear(); // paranoid check
				
				for( int iTest = 0; iTest < testCount; ++iTest )
				{
					DiaQDecisionTest eval = new DiaQDecisionTest();

					parser.Reader.ReadToFollowing( "eval" );
					string testTypeStr = parser.Reader.GetAttribute( "testType" );
					string varTypeStr = parser.Reader.GetAttribute( "varType" );
					string valTypeStr = parser.Reader.GetAttribute( "valType" );
					
					eval.testType = (DiaQDecisionTest.TestType) System.Enum.Parse( typeof(DiaQDecisionTest.TestType), testTypeStr );
					eval.varType = (DiaQDecisionTest.VarType) System.Enum.Parse( typeof(DiaQDecisionTest.VarType), varTypeStr );
					eval.valType = (DiaQDecisionTest.ValType) System.Enum.Parse( typeof(DiaQDecisionTest.ValType), valTypeStr );

					parser.Reader.ReadToFollowing( "opt1" );
					int sopt1Count = int.Parse( parser.Reader.GetAttribute( "count" ) );
					eval.s_opt1 = new string[sopt1Count];
					for( int iso1 = 0; iso1 < sopt1Count; ++iso1 )
					{
						parser.Reader.ReadToFollowing( "value" );
						eval.s_opt1[iso1] = parser.Reader.GetText();
					}
					
					parser.Reader.ReadToFollowing( "opt2" );
					int sopt2Count = int.Parse( parser.Reader.GetAttribute( "count" ) );
					eval.s_opt2 = new string[sopt2Count];
					for( int iso2 = 0; iso2 < sopt2Count; ++iso2 )
					{
						parser.Reader.ReadToFollowing( "value" );
						eval.s_opt2[iso2] = parser.Reader.GetText();
					}
		...
		parser.Reader.ReadToFollowing( "link_list" );
		int linkCount = int.Parse( parser.Reader.GetAttribute( "count" ) );
		for( int iNode = 0; iNode < linkCount; ++iNode )
		{
			parser.Reader.ReadToFollowing( "node_links" );
			int numLinks = int.Parse( parser.Reader.GetAttribute( "count" ) );
			int originNodeID = int.Parse( parser.Reader.GetAttribute( "origin" ) );
			DiaQNode originNode = graph.GetNode( originNodeID );
			originNode.outputs.Clear();

			for( int iLink = 0; iLink < numLinks; ++iLink )
			{
				parser.Reader.ReadToFollowing( "link" );
				DiaQNode.NodeLink link = new DiaQNode.NodeLink();

				link.outSlot = int.Parse(  parser.Reader.GetAttribute( "slot" ) );
				link.targetNodeId = int.Parse(  parser.Reader.GetAttribute( "target" ) );
				originNode.outputs.Add( link );
			}
		}
	...
	public static void ExportGraph( DiaQGraph graph, string path = "" )
	{
		if( null == graph )
			return;
			...
			// If there is no default path, open a save dialog
			if( path.Length == 0 )
				path = EditorUtility.SaveFilePanel( "Export XML", "", graph.name, "xml" );

			// If the user didn't specify a proper path, cancel
			if( path.Length == 0 )
				return;
			
			Zephyr.XML parser = new Zephyr.XML( path, Zephyr.XML.ParseType.Write );
			
			parser.Writer.StartDocument();

			// Start the graph output
			parser.Writer.WriteElement( "graph" );
			parser.Writer.WriteAttribute( "name", graph.name );
			parser.Writer.WriteAttribute( "customIdent", graph.customIdent );
			parser.Writer.WriteAttribute( "playerSeen", graph.HasSeenPlayer.ToString() );
				
			// Start the node list output
			parser.Writer.WriteElement( "node_list" );
			parser.Writer.WriteAttribute( "count", graph.nodes.Count.ToString() );
			foreach( DiaQNode node in graph.nodes )
			{
				parser.Writer.WriteElement( "node" );
				parser.Writer.WriteAttribute( "id", node.id.ToString() );
				parser.Writer.WriteAttribute( "x", node.position.x.ToString() );
				parser.Writer.WriteAttribute( "y", node.position.y.ToString() );
				parser.Writer.WriteAttribute( "w", node.position.width.ToString() );
				parser.Writer.WriteAttribute( "h", node.position.height.ToString() );
				parser.Writer.WriteAttribute( "type", node.type.ToString() );
				parser.Writer.WriteAttribute( "comment", node.comment );

				parser.Writer.WriteElement( "data" );
				parser.Writer.WriteAttribute( "count", node.data.Length.ToString() );
				foreach( string val in node.data )
				{
					parser.Writer.WriteElement( "value" );
					parser.Writer.WriteText( val );
					parser.Writer.EndElement();
				}
				parser.Writer.EndElementFull();
			
				parser.Writer.WriteElement( "toggle" );
				parser.Writer.WriteAttribute( "count", node.toggle.Length.ToString() );
				foreach( bool val in node.toggle )
				{
					parser.Writer.WriteElement( "value" );
					parser.Writer.WriteText( val ? "true" : "false" );
					parser.Writer.EndElement();
				}
				parser.Writer.EndElementFull();
			
				parser.Writer.WriteElement( "i_data" );
				parser.Writer.WriteAttribute( "count", node.i_data.Length.ToString() );
				foreach( int val in node.i_data )
				{
					parser.Writer.WriteElement( "value" );
					parser.Writer.WriteText( val.ToString() );
					parser.Writer.EndElement();
				}
				parser.Writer.EndElementFull();

				parser.Writer.WriteElement( "f_data" );
				parser.Writer.WriteAttribute( "count", node.f_data.Length.ToString() );
				foreach( float val in node.f_data )
				{
					parser.Writer.WriteElement( "value" );
					parser.Writer.WriteText( val.ToString() );
					parser.Writer.EndElement();
				...
				parser.Writer.WriteElement( "choices" );
				parser.Writer.WriteAttribute( "count", node.choices.Count.ToString() );
				foreach( string choice in node.choices )
				{
					parser.Writer.WriteElement( "value" );
					parser.Writer.WriteText( choice );
					parser.Writer.EndElement();
				}
				parser.Writer.EndElementFull();
				...
				if( tests != null )
				{
					parser.Writer.WriteElement( "tests" );
					parser.Writer.WriteAttribute( "count", tests.Count.ToString() );
					foreach( DiaQDecisionTest test in tests )
					{
						parser.Writer.WriteElement( "eval" );
						parser.Writer.WriteAttribute( "testType", test.testType.ToString() );
						parser.Writer.WriteAttribute( "varType", test.varType.ToString() );
						parser.Writer.WriteAttribute( "valType", test.valType.ToString() );
					
						parser.Writer.WriteElement( "opt1" );
						parser.Writer.WriteAttribute( "count", test.s_opt1.Length.ToString() );
						foreach( string val in test.s_opt1 )
						{
							parser.Writer.WriteElement( "value" );
							parser.Writer.WriteText( val );
							parser.Writer.EndElement();
						...
						parser.Writer.WriteElement( "opt2" );
						parser.Writer.WriteAttribute( "count", test.s_opt2.Length.ToString() );
						foreach( string val in test.s_opt2 )
						{
							parser.Writer.WriteElement( "value" );
							parser.Writer.WriteText( val );
							parser.Writer.EndElement();
						...
						parser.Writer.EndElementFull();	// eval
					}
			...
			parser.Writer.EndElement();		// </node_list>
		
			parser.Writer.WriteElement( "link_list" );
			parser.Writer.WriteAttribute( "count", graph.nodes.Count().ToString() );
			foreach( DiaQNode node in graph.nodes )
			{
				parser.Writer.WriteElement( "node_links" );	
				parser.Writer.WriteAttribute( "count", node.outputs.Count().ToString() );
				parser.Writer.WriteAttribute( "origin", node.id.ToString() );

				foreach( DiaQNode.NodeLink link in node.outputs )
				{
					parser.Writer.WriteElement( "link" );
					parser.Writer.WriteAttribute( "slot", link.outSlot.ToString() );
					parser.Writer.WriteAttribute( "target", link.targetNodeId.ToString() );
					parser.Writer.EndElement();
				}
				parser.Writer.EndElementFull();
			}
			...
			parser.Writer.EndElement();		// </graph>
			parser.Writer.EndDocument();

			parser.Close();
		}

		protected static DiaQGraph FindGraphByName( string name )
		{
			foreach( DiaQGraph assetGraph in asset.graphs )
				if( assetGraph.name == name )
					return assetGraph;

			return null;
		}

		protected static DiaQGraph CreateEmptyGraph( string name )
		{
			// new dialogue graph
			DiaQGraph graph = new DiaQGraph();
			graph.name = name;
		...
		private void DuplicateCurrentGraph()
		{
			...
			{
				// new dialogue graph
				DiaQGraph newGraph = new DiaQGraph();
				newGraph.name = currGraph.name + " (copy)";
				newGraph.nodes = currGraph.nodes.ToList();
				asset.graphs.Add(newGraph);
				currGraph = newGraph;
				EditorUtility.SetDirty(asset);
			}
			else
			{	// new quest
				DiaQuest newQuest = new DiaQuest();
				newQuest.name = currQuest.name + " (copy)";
				newQuest.conditions = currQuest.conditions.ToList();
				newQuest.rewards = currQuest.rewards.ToList();
				newQuest.text = currQuest.text;
				currQuest = newQuest;
				asset.quests.Add(newQuest);
				EditorUtility.SetDirty(asset);
			}
		}
		...
		private Color GetLinkColour(DiaQNode n, int slot)
		{
			...
			else if( n.type == DiaQNode.Type.DecisionTree )
			{
				if( slot == n.decisionTree.tests.Count() )
					return DiaQEdGUI.Col_LinkRed;
				else
					return DiaQEdGUI.Col_LinkGreen;
			}
